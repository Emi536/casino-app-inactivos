...
# SECCIÓN 3: FILTRO POR DÍAS DE INACTIVIDAD
    ...
    if archivo:
        ...
        if df is not None:
            ...
            df_filtrado = pd.DataFrame(resumen)
            df_filtrado = df_filtrado.sort_values("Días inactivo", ascending=False)

            st.subheader(f"👥 Jugadores que no cargan hace al menos {min_dias} días")
            st.dataframe(df_filtrado)

            df_filtrado.to_excel("jugadores_filtrados_por_dias.xlsx", index=False)
